...
void * macacoAB(void * a) {
	...
	while(1){
		...
		if (macacosAB == 1) pthread_mutex_lock(&lock_corda);
		...
		if (macacosAB == 0) pthread_mutex_unlock(&lock_corda);
...
void * macacoBA(void * a) {
	...
	while(1){
		...
		if(macacosBA == 1) pthread_mutex_lock(&lock_corda);
		...
		if(macacosBA == 0) pthread_mutex_unlock(&lock_corda);
